...
// Persistent session store: token → { userId, role, name }
let sessions = new Map();
// Admin sessions: token → true (admin sessions stay in-memory, they use env password)
const adminSessions = new Map();

// Load sessions from disk, prune orphans, save on every change
async function loadSessions() {
  ...
  // Remove sessions whose user no longer exists in users.json
...
const AVATAR_OPTIONS = {
  ...
  backgroundColor: null // any valid 6-char hex
};
...
function isConnectedPair(teams, userA, userB) {
  return teams.some(t =>
    t.status === 'active' && (
      (t.bossId === userA && t.workerId === userB) ||
      (t.bossId === userB && t.workerId === userA)
    )
  );
}
    ...
    try {
      ...
      if (msg.type === 'auth') {
        ...
        if (session) {
          ...
          const [tasks, team, requests, timelog, users] = await Promise.all([
          ...
          let myTeam = team.filter(t =>
            ...
            ...t,
            bossName: userMap.get(t.bossId)?.name || 'Unknown',
            workerName: userMap.get(t.workerId)?.name || 'Unknown',
            bossAvatarUrl: avatarUrlFromUser(userMap.get(t.bossId)),
            workerAvatarUrl: avatarUrlFromUser(userMap.get(t.workerId))
          }));
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    res.status(201).json({
      token,
      user: { id: user.id, name: user.name, role: user.role, avatarUrl: avatarUrlFromUser(user) }
    });
  } catch (err) {
...
app.get('/api/auth/me', authMiddleware, async (req, res) => {
  try {
    ...
    res.json({ id: user.id, name: user.name, role: user.role, avatarUrl: avatarUrlFromUser(user) });
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
app.get('/api/workers', authMiddleware, async (req, res) => {
  try {
    ...
    const workers = users
      ...
      .map(u => ({ id: u.id, name: u.name, avatarUrl: avatarUrlFromUser(u) }));
    res.json(workers);
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
  }
...
app.get('/api/team', authMiddleware, async (req, res) => {
  try {
    ...
    const result = myTeam.map(t => ({
      ...
      bossAvatarUrl: avatarUrlFromUser(users.find(u => u.id === t.bossId)),
      workerAvatarUrl: avatarUrlFromUser(users.find(u => u.id === t.workerId))
    }));
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: 'Internal server error' });
...
app.get('/api/chat/conversations', authMiddleware, async (req, res) => {
  try {
    ...
    // Seed all active boss-worker connections so chat list is never empty after connection
    const connectedUserIds = new Set();
    for (const t of teams) {
      if (t.status !== 'active') continue;
      if (t.bossId === req.user.userId) connectedUserIds.add(t.workerId);
      if (t.workerId === req.user.userId) connectedUserIds.add(t.bossId);
    }
    for (const otherId of connectedUserIds) {
      const other = userMap.get(otherId);
      if (!other) continue;
      convMap[otherId] = {
        userId: otherId,
        userName: other.name,
        userAvatarUrl: avatarUrlFromUser(other),
        unread: 0,
        lastMessage: null
      };
    }
...
app.get('/api/chat/:userId', authMiddleware, async (req, res) => {
  try {
    const teams = await storage.read('teams.json');
    if (!isConnectedPair(teams, req.user.userId, req.params.userId)) {
      return res.status(403).json({ error: 'You can only chat with connected team members' });
    }
